...
const transitions: Record<string,string[]> = { Pending:["Confirmed","Cancelled"], Confirmed:["Processing","Cancelled"], Processing:["Packed","Cancelled"], Packed:["Shipped"], Shipped:["Out For Delivery"], "Out For Delivery":["Delivered"], Delivered:[], Cancelled:[] };
export const updateOrder=async(id:string,input:UpdateOrderInput)=>{const order=await getOrder(id);if(input.orderStatus && !transitions[order.orderStatus]?.includes(input.orderStatus))throw new Error(`Invalid order status transition: ${order.orderStatus} to ${input.orderStatus}`);if(order.orderStatus === "Cancelled" || order.orderStatus === "Delivered")throw new Error("Order can no longer be updated");const updated=await repository.updateOrder(id,input);if(!updated)throw new Error("Order not found");await publishOrderEvent("order.updated",updated);return updated;};
